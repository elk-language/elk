...
// Macros can be inherited, so we need to resolve the
// inheritance in a simple way (ignoring generics etc) without
// reporting errors, because not all types may be defined
// (some types are yet to be generated by macros).
//
// We define the parents in this stage with TemporaryParent.
// We will resolve the inheritance chain fully with error reporting
// after top-level macro expansion.
...
func (c *Checker) checkTypeDefinition(typedefCheck *typeDefinitionCheck, location *position.Location) bool {
	...
	c.removeTemporaryParents(typedefCheck.typ)
...
superclassSwitch:
	...
	temp := types.NewTemporaryParent(superclass)
