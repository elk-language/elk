...
func TestMultiplication(t *testing.T) {
	tests := testTable{
		...
		"cannot have newlines before the operator": {
			...
			err: error.ErrorList{
				error.NewFailure(L("<main>", P(2, 2, 1), P(2, 2, 1)), "unexpected *, expected an expression"),
				error.NewFailure(L("<main>", P(6, 3, 1), P(6, 3, 1)), "unexpected *, expected an expression"),
			},
		},
...
func TestUnaryExpressions(t *testing.T) {
	tests := testTable{
		"plus cannot be nested without spaces": {
			...
			err: error.ErrorList{
				error.NewFailure(L("<main>", P(0, 1, 1), P(1, 1, 2)), "unexpected ++, expected an expression"),
			},
		},
...
func TestExponentiation(t *testing.T) {
	tests := testTable{
		...
		"cannot have newlines before the operator": {
			...
			err: error.ErrorList{
				error.NewFailure(L("<main>", P(2, 2, 1), P(3, 2, 2)), "unexpected **, expected an expression"),
				error.NewFailure(L("<main>", P(7, 3, 1), P(8, 3, 2)), "unexpected **, expected an expression"),
			},
		},
