...
func TestString(t *testing.T) {
	tests := testTable{
		...
		"processes escape sequences": {
			...
			want: []*token.Token{
				T(S(P(0, 1, 1), P(0, 1, 1)), token.STRING_BEG),
				V(S(P(1, 1, 2), P(93, 1, 94)), token.STRING_CONTENT, "Some \n a\\wesome \t str\\ing \r with \\ escape \b sequences \"\v\f\x12\a\u00e9\U0010FFFF$#"),
				T(S(P(94, 1, 95), P(94, 1, 95)), token.STRING_END),
			},
		},
...
and really useful"`,
		...
		"can be interpolated": {
			...
			want: []*token.Token{
				...
				V(S(P(31, 1, 32), P(34, 1, 35)), token.PUBLIC_IDENTIFIER, "wita"),
		...
		"can contain short local and constant interpolation": {
			...
			want: []*token.Token{
				T(S(P(0, 1, 1), P(0, 1, 1)), token.STRING_BEG),
				V(S(P(1, 1, 2), P(4, 1, 5)), token.STRING_INTERP_LOCAL, "foo"),
				V(S(P(5, 1, 6), P(6, 1, 7)), token.STRING_CONTENT, ", "),
				V(S(P(7, 1, 8), P(11, 1, 12)), token.STRING_INTERP_LOCAL, "_foo"),
				V(S(P(12, 1, 13), P(13, 1, 14)), token.STRING_CONTENT, ", "),
				V(S(P(14, 1, 15), P(17, 1, 18)), token.STRING_INTERP_CONSTANT, "Bar"),
				V(S(P(18, 1, 19), P(22, 1, 23)), token.STRING_CONTENT, " and "),
				V(S(P(23, 1, 24), P(27, 1, 28)), token.STRING_INTERP_CONSTANT, "_Bar"),
				T(S(P(28, 1, 29), P(28, 1, 29)), token.STRING_END),
			},
		},
