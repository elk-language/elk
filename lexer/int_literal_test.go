...
func TestInt(t *testing.T) {
	tests := testTable{
		...
		"decimal with leading zeros": {
			input: "00015",
			want: []*token.Token{
				V(P(0, 5, 1, 1), token.DEC_INT, "00015"),
			},
		},
		"decimal with underscores": {
			input: "23_200_123",
			want: []*token.Token{
				V(P(0, 10, 1, 1), token.DEC_INT, "23200123"),
			},
		},
		"decimal can't begin with underscores": {
			...
			want: []*token.Token{
				...
				V(P(1, 10, 1, 2), token.DEC_INT, "23200123"),
			},
		},
		...
		"hex": {
			input: "0x354ab1",
			want: []*token.Token{
				V(P(0, 8, 1, 1), token.HEX_INT, "354ab1"),
			},
		},
		"hex with underscores": {
			input: "0x35_4a_b1",
			want: []*token.Token{
				V(P(0, 10, 1, 1), token.HEX_INT, "354ab1"),
			},
		},
		"leading zeros invalidate hex": {
			input: "00x21",
			want: []*token.Token{
				V(P(0, 2, 1, 1), token.DEC_INT, "00"),
		...
		"hex ends on last valid character": {
			input: "0x354fp",
			want: []*token.Token{
				V(P(0, 6, 1, 1), token.HEX_INT, "354f"),
		...
		"octal": {
			input: "0o603",
			want: []*token.Token{
				V(P(0, 5, 1, 1), token.OCT_INT, "603"),
			},
		},
		"octal with underscores": {
			input: "0o3201_5200",
			want: []*token.Token{
				V(P(0, 11, 1, 1), token.OCT_INT, "32015200"),
			},
		},
		...
		"quaternary": {
			input: "0q30212",
			want: []*token.Token{
				V(P(0, 7, 1, 1), token.QUAT_INT, "30212"),
			},
		},
		"quaternary with underscores": {
			input: "0q3201200_23010000",
			want: []*token.Token{
				V(P(0, 18, 1, 1), token.QUAT_INT, "320120023010000"),
			},
		},
		...
		"binary": {
			input: "0b1010",
			want: []*token.Token{
				V(P(0, 6, 1, 1), token.BIN_INT, "1010"),
			},
		},
		"binary with underscores": {
			input: "0b10_10_10",
			want: []*token.Token{
				V(P(0, 10, 1, 1), token.BIN_INT, "101010"),
			},
		},
		...
		"binary without digits": {
			input: "0b",
			want: []*token.Token{
				V(P(0, 2, 1, 1), token.BIN_INT, ""),
			},
		},
		"duodecimal": {
			input: "0da12b3",
			want: []*token.Token{
				V(P(0, 7, 1, 1), token.DUO_INT, "a12b3"),
			},
		},
		...
		"duodecimal ends on last valid character": {
			input: "0d23a3bca3",
			want: []*token.Token{
				V(P(0, 7, 1, 1), token.DUO_INT, "23a3b"),
