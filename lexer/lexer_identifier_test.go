...
func TestInstanceVariable(t *testing.T) {
	tests := testTable{
		...
		"may start with a digit": {
			input: "@1",
			want: []*token.Token{
				V(S(P(0, 1, 1), P(1, 1, 2)), token.INSTANCE_VARIABLE, "1"),
			},
		},
...
func TestSpecialIdentifier(t *testing.T) {
	tests := testTable{
		"ends on the last valid character": {
			input: "$foo:+",
			want: []*token.Token{
				V(S(P(0, 1, 1), P(3, 1, 4)), token.SPECIAL_IDENTIFIER, "foo"),
				T(S(P(4, 1, 5), P(4, 1, 5)), token.COLON),
				T(S(P(5, 1, 6), P(5, 1, 6)), token.PLUS),
			},
		},
		"may contain letters underscores and numbers": {
			input: "$some_ivar123",
			want: []*token.Token{
				V(S(P(0, 1, 1), P(12, 1, 13)), token.SPECIAL_IDENTIFIER, "some_ivar123"),
			},
		},
		"may start with an uppercase letter": {
			input: "$SomeIvar123",
			want: []*token.Token{
				V(S(P(0, 1, 1), P(11, 1, 12)), token.SPECIAL_IDENTIFIER, "SomeIvar123"),
			},
		},
		"may start with a digit": {
			input: "$1",
			want: []*token.Token{
				V(S(P(0, 1, 1), P(1, 1, 2)), token.SPECIAL_IDENTIFIER, "1"),
			},
		},
		"may start with an underscore": {
			input: "$_bar",
			want: []*token.Token{
				V(S(P(0, 1, 1), P(4, 1, 5)), token.SPECIAL_IDENTIFIER, "_bar"),
			},
		},
		"may start with a utf-8 character": {
			input: "$łódź",
			want: []*token.Token{
				V(S(P(0, 1, 1), P(7, 1, 5)), token.SPECIAL_IDENTIFIER, "łódź"),
			},
		},
		"may contain utf-8 characters": {
			input: "$zażółć_gęślą_jaźń + 2",
			want: []*token.Token{
				V(S(P(0, 1, 1), P(26, 1, 18)), token.SPECIAL_IDENTIFIER, "zażółć_gęślą_jaźń"),
				T(S(P(28, 1, 20), P(28, 1, 20)), token.PLUS),
				V(S(P(30, 1, 22), P(30, 1, 22)), token.INT, "2"),
			},
		},
