...
and really useful"`,
		...
		"can be interpolated": {
			...
			want: []*Token{
				...
				V(PublicIdentifierToken, "interpolated", 8, 12, 1, 9),
				T(StringInterpEndToken, 20, 1, 1, 21),
				V(StringContentToken, " string ", 21, 8, 1, 22),
				T(StringInterpBegToken, 29, 2, 1, 30),
				V(PublicIdentifierToken, "with", 31, 4, 1, 32),
				T(DotToken, 35, 1, 1, 36),
				V(PublicIdentifierToken, "expressions", 36, 11, 1, 37),
		...
		"does not generate unnecessary tokens when interpolation is right beside delimiters": {
			...
			want: []*Token{
				...
				V(PublicIdentifierToken, "interpolated", 3, 12, 1, 4),
		...
		"raw strings can be nested in string interpolation": {
			...
			want: []*Token{
				...
				V(PublicIdentifierToken, "baz", 7, 3, 1, 8),
