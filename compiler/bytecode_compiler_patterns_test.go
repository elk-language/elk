package compiler_test

import (
	"testing"

	"github.com/elk-language/elk/bitfield"
	"github.com/elk-language/elk/bytecode"
	"github.com/elk-language/elk/value"
	"github.com/elk-language/elk/vm"
)

func TestBytecodeSwitch(t *testing.T) {
	tests := bytecodeTestTable{
		"with a few literal cases": {
			input: `
			  var a: any = 0
				switch a
				case true then "a"
				case false then "b"
				case 0 then "c"
				case 1 then "d"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.TRUE),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 35,
					byte(bytecode.DUP),
					byte(bytecode.FALSE),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 24,
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.JUMP), 0, 13,
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(128, 8, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(5, 11),
					bytecode.NewLineInfo(6, 11),
					bytecode.NewLineInfo(7, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(8, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
					value.Ref(value.String("b")),
					value.Ref(value.String("c")),
					value.Ref(value.String("d")),
				},
			),
		},
		"with else": {
			input: `
			  var a: any = 0
				switch a
				case true then "a"
				case false then "b"
				else "c"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),

					byte(bytecode.DUP),
					byte(bytecode.TRUE),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 13,

					byte(bytecode.DUP),
					byte(bytecode.FALSE),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,

					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(101, 7, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(5, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(6, 1),
					bytecode.NewLineInfo(7, 1),
				},
				[]value.Value{
					value.Ref(value.String("a")),
					value.Ref(value.String("b")),
					value.Ref(value.String("c")),
				},
			),
		},
		"literal true": {
			input: `
			  var a: any = 0
				switch a
				case true then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.TRUE),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal false": {
			input: `
			  var a: any = 0
				switch a
				case false then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.FALSE),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(65, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},

		"literal nil": {
			input: `
			  var a: any = 0
				switch a
				case nil then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.NIL),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(63, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal string": {
			input: `
			  var a: any = 0
				switch a
				case "foo" then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(65, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("foo")),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal raw string": {
			input: `
			  var a: any = 0
				switch a
				case 'foo' then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(65, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("foo")),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal interpolated string": {
			input: `
			  var a: any = 0
				switch a
				case "f${a}" then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.NEW_STRING8), 2,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(67, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 14),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("f")),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal symbol": {
			input: `
			  var a: any = 0
				switch a
				case :foo then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.ToSymbol("foo").ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal interpolated symbol": {
			input: `
			  var a: any = 0
				switch a
				case :"f${a}" then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.NEW_SYMBOL8), 2,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(68, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 14),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("f")),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal int": {
			input: `
			  var a: any = 0
				switch a
				case 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(61, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal int64": {
			input: `
			  var a: any = 0
				switch a
				case 5i64 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT64_8), 5,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal uint64": {
			input: `
			  var a: any = 0
				switch a
				case 5u64 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_UINT64_8), 5,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal int32": {
			input: `
			  var a: any = 0
				switch a
				case 5i32 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT32_8), 5,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal uint32": {
			input: `
			  var a: any = 0
				switch a
				case 5u32 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_UINT32_8), 5,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal int16": {
			input: `
			  var a: any = 0
				switch a
				case 5i16 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT16_8), 5,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal uint16": {
			input: `
			  var a: any = 0
				switch a
				case 5u16 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_UINT16_8), 5,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal int8": {
			input: `
			  var a: any = 0
				switch a
				case 5i8 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT8), 5,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(63, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal uint8": {
			input: `
			  var a: any = 0
				switch a
				case 5u8 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_UINT8), 5,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(63, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"literal float": {
			input: `
			  var a: any = 0
				switch a
				case 5.8 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(63, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Float(5.8).ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal float64": {
			input: `
			  var a: any = 0
				switch a
				case 5.8f64 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(66, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Float64(5.8).ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal float32": {
			input: `
			  var a: any = 0
				switch a
				case 5.8f32 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(66, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Float32(5.8).ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal negative float32": {
			input: `
			  var a: any = 0
				switch a
				case -5.8f32 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(67, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Float32(-5.8).ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"literal big float": {
			input: `
			  var a: any = 0
				switch a
				case 5.8bf then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(65, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.NewBigFloat(5.8)),
					value.Ref(value.String("a")),
				},
			),
		},
		"root constant": {
			input: `
				const Foo = 3
			  var a: any = 0
				switch a
				case ::Foo then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.GET_CONST8), 0,
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.INT_3),
					byte(bytecode.DEF_CONST),
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_CONST8), 1,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(83, 6, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(2, 7),
					bytecode.NewLineInfo(1, 0),
					bytecode.NewLineInfo(3, 2),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(5, 12),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(6, 2),
				},
				[]value.Value{
					value.ToSymbol("Root").ToValue(),
					value.ToSymbol("Foo").ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"constant lookup": {
			input: `
				module Foo
					const Bar = 3
				end
			  var a: any = 0
				switch a
				case ::Foo::Bar then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EXEC),
					byte(bytecode.POP),
					byte(bytecode.GET_CONST8), 1,
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.INT_3),
					byte(bytecode.DEF_CONST),
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_CONST8), 3,
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(112, 8, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 5),
					bytecode.NewLineInfo(3, 5),
					bytecode.NewLineInfo(1, 0),
					bytecode.NewLineInfo(5, 2),
					bytecode.NewLineInfo(6, 1),
					bytecode.NewLineInfo(7, 13),
					bytecode.NewLineInfo(6, 1),
					bytecode.NewLineInfo(8, 2),
				},
				[]value.Value{
					value.Ref(vm.NewBytecodeFunctionNoParams(
						value.ToSymbol("<namespaceDefinitions>"),
						[]byte{
							byte(bytecode.GET_CONST8), 0,
							byte(bytecode.LOAD_VALUE_1),
							byte(bytecode.DEF_NAMESPACE), 0,
							byte(bytecode.NIL),
							byte(bytecode.RETURN),
						},
						L(P(0, 1, 1), P(112, 8, 8)),
						bytecode.LineInfoList{
							bytecode.NewLineInfo(1, 5),
							bytecode.NewLineInfo(8, 2),
						},
						[]value.Value{
							value.ToSymbol("Root").ToValue(),
							value.ToSymbol("Foo").ToValue(),
						},
					)),
					value.ToSymbol("Foo").ToValue(),
					value.ToSymbol("Bar").ToValue(),
					value.ToSymbol("Foo::Bar").ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"negative constant lookup": {
			input: `
				module Foo
					const Bar = 3
				end
			  var a: any = 0
				switch a
				case -::Foo::Bar then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EXEC),
					byte(bytecode.POP),
					byte(bytecode.GET_CONST8), 1,
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.INT_3),
					byte(bytecode.DEF_CONST),
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_CONST8), 3,
					byte(bytecode.NEGATE_INT),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(113, 8, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 5),
					bytecode.NewLineInfo(3, 5),
					bytecode.NewLineInfo(1, 0),
					bytecode.NewLineInfo(5, 2),
					bytecode.NewLineInfo(6, 1),
					bytecode.NewLineInfo(7, 14),
					bytecode.NewLineInfo(6, 1),
					bytecode.NewLineInfo(8, 2),
				},
				[]value.Value{
					value.Ref(vm.NewBytecodeFunctionNoParams(
						value.ToSymbol("<namespaceDefinitions>"),
						[]byte{
							byte(bytecode.GET_CONST8), 0,
							byte(bytecode.LOAD_VALUE_1),
							byte(bytecode.DEF_NAMESPACE), 0,
							byte(bytecode.NIL),
							byte(bytecode.RETURN),
						},
						L(P(0, 1, 1), P(113, 8, 8)),
						bytecode.LineInfoList{
							bytecode.NewLineInfo(1, 5),
							bytecode.NewLineInfo(8, 2),
						},
						[]value.Value{
							value.ToSymbol("Root").ToValue(),
							value.ToSymbol("Foo").ToValue(),
						},
					)),
					value.ToSymbol("Foo").ToValue(),
					value.ToSymbol("Bar").ToValue(),
					value.ToSymbol("Foo::Bar").ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"less pattern": {
			input: `
			  var a: any = 0
				switch a
				case < 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(63, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 23),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"less than root constant": {
			input: `
				const Foo = 5
			  var a: any = 0
				switch a
				case < ::Foo then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.GET_CONST8), 0,
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.INT_5),
					byte(bytecode.DEF_CONST),
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_CONST8), 1,
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(85, 6, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(2, 7),
					bytecode.NewLineInfo(1, 0),
					bytecode.NewLineInfo(3, 2),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(5, 24),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(6, 2),
				},
				[]value.Value{
					value.ToSymbol("Root").ToValue(),
					value.ToSymbol("Foo").ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"less than negative root constant": {
			input: `
				const Foo = 2
			  var a: any = 0
				switch a
				case < -::Foo then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.GET_CONST8), 0,
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.INT_2),
					byte(bytecode.DEF_CONST),
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_CONST8), 1,
					byte(bytecode.NEGATE_INT),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(86, 6, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(2, 7),
					bytecode.NewLineInfo(1, 0),
					bytecode.NewLineInfo(3, 2),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(5, 25),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(6, 2),
				},
				[]value.Value{
					value.ToSymbol("Root").ToValue(),
					value.ToSymbol("Foo").ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"less equal pattern": {
			input: `
			  var a: any = 0
				switch a
				case <= 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS_EQUAL),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 23),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"greater pattern": {
			input: `
			  var a: any = 0
				switch a
				case > 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.GREATER),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(63, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 23),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"greater equal pattern": {
			input: `
			  var a: any = 0
				switch a
				case >= 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.GREATER_EQUAL),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 23),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"equal pattern": {
			input: `
			  var a: any = 0
				switch a
				case == 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"equal regex pattern": {
			input: `
			  var a: any = 0
				switch a
				case == %/fo+/ then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(69, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.MustCompileRegex("fo+", bitfield.BitField8{})),
					value.Ref(value.String("a")),
				},
			),
		},
		"equal local pattern": {
			input: `
			  var a: any = 0
				b := 2
				switch a
				case == b then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 2,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.INT_2),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL_2),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(75, 6, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 2),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(5, 11),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(6, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"not equal pattern": {
			input: `
			  var a: any = 0
				switch a
				case != 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.NOT_EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"lax equal pattern": {
			input: `
			  var a: any = 0
				b := 2
				switch a
				case =~ b then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 2,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.INT_2),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL_2),
					byte(bytecode.LAX_EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),

					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(75, 6, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 2),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(5, 11),
					bytecode.NewLineInfo(4, 1),
					bytecode.NewLineInfo(6, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"lax not equal pattern": {
			input: `
			  var a: any = 0
				switch a
				case !~ 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.LAX_NOT_EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(64, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"strict equal pattern": {
			input: `
			  var a: any = 0
				switch a
				case === 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.STRICT_EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(65, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"strict not equal pattern": {
			input: `
			  var a: any = 0
				switch a
				case !== 5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.STRICT_NOT_EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(65, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 11),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},
		"strict not equal negative pattern": {
			input: `
			  var a: any = 0
				switch a
				case !== -5 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 0xFB,
					byte(bytecode.STRICT_NOT_EQUAL),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(66, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("a")),
				},
			),
		},

		"regex pattern": {
			input: `
			  a := "foo"
				switch a
				case %/fo+/ then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.SWAP),
					byte(bytecode.CALL_METHOD8), 2,
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(62, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 13),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.String("foo")),
					value.Ref(value.MustCompileRegex("fo+", bitfield.BitField8{})),
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("matches"), 1)),
					value.Ref(value.String("a")),
				},
			),
		},
		"variable pattern": {
			input: `
			  a := 0
				switch a
				case n then n + 2
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 2,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 7,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_2),
					byte(bytecode.INT_2),
					byte(bytecode.ADD_INT),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(55, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 13),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{},
			),
		},
		"range": {
			input: `
			  var a: any = 0
				switch a
				case -2...9 then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SWAP),
					byte(bytecode.CALL_METHOD8), 1,
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(66, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 13),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(value.NewClosedRange(value.SmallInt(-2).ToValue(), value.SmallInt(9).ToValue())),
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("#contains"), 1)),
					value.Ref(value.String("a")),
				},
			),
		},
		"range with constants": {
			input: `
				const Foo = 3
				const Bar = 10
			  var a: any = 0
				switch a
				case ::Foo...-::Bar then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.GET_CONST8), 0,
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.INT_3),
					byte(bytecode.DEF_CONST),
					byte(bytecode.GET_CONST8), 0,
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.LOAD_INT_8), 10,
					byte(bytecode.DEF_CONST),
					byte(bytecode.INT_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_CONST8), 1,
					byte(bytecode.GET_CONST8), 2,
					byte(bytecode.NEGATE_INT),
					byte(bytecode.NEW_RANGE), 0,
					byte(bytecode.SWAP),
					byte(bytecode.CALL_METHOD8), 3,
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(111, 7, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(2, 7),
					bytecode.NewLineInfo(3, 6),
					bytecode.NewLineInfo(1, 0),
					bytecode.NewLineInfo(4, 2),
					bytecode.NewLineInfo(5, 1),
					bytecode.NewLineInfo(6, 20),
					bytecode.NewLineInfo(5, 1),
					bytecode.NewLineInfo(7, 2),
				},
				[]value.Value{
					value.ToSymbol("Root").ToValue(),
					value.ToSymbol("Foo").ToValue(),
					value.ToSymbol("Bar").ToValue(),
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("#contains"), 1)),
					value.Ref(value.String("a")),
				},
			),
		},
		"set pattern": {
			input: `
			  a := ^[1, 5, -4]
				switch a
				case ^[1, _, -4] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 28,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_3),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS_NP), 0, 19,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.JUMP_UNLESS_NP), 0, 11,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 252,
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(73, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 43),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(vm.MustNewHashSetOfValueWithElements(nil, (value.SmallInt(5)).ToValue(), (value.SmallInt(1)).ToValue(), (value.SmallInt(-4)).ToValue())),
					value.Ref(value.SetMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(value.String("a")),
				},
			),
		},
		"set pattern with rest elements": {
			input: `
			  a := ^[1, 5, -4]
				switch a
				case ^[1, *, -4] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 28,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.GREATER_EQUAL),
					byte(bytecode.JUMP_UNLESS_NP), 0, 19,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.JUMP_UNLESS_NP), 0, 11,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 252,
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(73, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 43),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(vm.MustNewHashSetOfValueWithElements(nil, (value.SmallInt(5)).ToValue(), (value.SmallInt(1)).ToValue(), (value.SmallInt(-4)).ToValue())),
					value.Ref(value.SetMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(value.String("a")),
				},
			),
		},

		"word set pattern": {
			input: `
			  a := ^['foo', 'bar']
				switch a
				case ^w[foo bar] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 28,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS_NP), 0, 19,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.JUMP_UNLESS_NP), 0, 11,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.CALL_METHOD_NT8), 6,
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 7,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(77, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 43),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					vm.NewNativeHashSetWithElements(
						value.String("foo"),
						value.String("bar"),
					).ToValue(),
					value.Ref(value.SetMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.HashSetClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("foo")),
					value.Ref(vm.NewNativeCallSiteInfo((value.HashSetClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(value.String("bar")),
					value.Ref(vm.NewNativeCallSiteInfo((value.HashSetClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(value.String("a")),
				},
			),
		},
		"symbol set pattern": {
			input: `
			  a := ^[:foo, :bar]
				switch a
				case ^s[foo bar] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 28,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS_NP), 0, 19,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.JUMP_UNLESS_NP), 0, 11,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.CALL_METHOD_NT8), 6,
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 7,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(75, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 43),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					vm.NewNativeHashSetWithElements(
						value.ToSymbol("foo"),
						value.ToSymbol("bar"),
					).ToValue(),
					value.Ref(value.SetMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.HashSetClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.ToSymbol("foo").ToValue(),
					value.Ref(vm.NewNativeCallSiteInfo((value.HashSetClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.ToSymbol("bar").ToValue(),
					value.Ref(vm.NewNativeCallSiteInfo((value.HashSetClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(value.String("a")),
				},
			),
		},
		"hex set pattern": {
			input: `
			  a := ^[0xff, 0x26]
				switch a
				case ^x[ff 26] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 30,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS_NP), 0, 21,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_16), 0, 255,
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.JUMP_UNLESS_NP), 0, 11,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 38,
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(73, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 45),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(vm.MustNewHashSetOfValueWithElements(nil, (value.SmallInt(38)).ToValue(), (value.SmallInt(255)).ToValue())),
					value.Ref(value.SetMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(value.String("a")),
				},
			),
		},

		"bin set pattern": {
			input: `
			  a := ^[0b11, 0b10]
				switch a
				case ^b[11 10] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 27,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL),
					byte(bytecode.JUMP_UNLESS_NP), 0, 18,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_3),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.JUMP_UNLESS_NP), 0, 10,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_2),
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(73, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 42),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(vm.MustNewHashSetOfValueWithElements(nil, (value.SmallInt(2)).ToValue(), (value.SmallInt(3)).ToValue())),
					value.Ref(value.SetMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("contains")).(*vm.NativeMethod), 1)),
					value.Ref(value.String("a")),
				},
			),
		},
		"list pattern": {
			input: `
			  a := [1, 5, [8, 3]]
				switch a
				case [1, < 8, [a, > 1 && < 5]] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 2,
					byte(bytecode.UNDEFINED),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.COPY),
					byte(bytecode.NEW_ARRAY_LIST8), 1,
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 124,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.INT_3),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 115,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 104,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 8,
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 80,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_2),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 64,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 55,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 44,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.GREATER),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 16,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(90, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 7),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 139),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(1)).ToValue(), (value.SmallInt(5)).ToValue()}),
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(8)).ToValue(), (value.SmallInt(3)).ToValue()}),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"word list pattern": {
			input: `
			  a := ['foo', 'bar']
				switch a
				case \w[foo bar] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 34,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 25,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 14,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(76, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 49),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.NativeArrayList[value.String]{value.String("foo"), value.String("bar")}),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("foo")),
					value.Ref(value.String("bar")),
					value.Ref(value.String("a")),
				},
			),
		},

		"symbol list pattern": {
			input: `
			  a := [:foo, :bar]
				switch a
				case \s[foo bar] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 34,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 25,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 14,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(74, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 49),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.NativeArrayList[value.Symbol]{value.ToSymbol("foo"), value.ToSymbol("bar")}),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.ToSymbol("foo").ToValue(),
					value.ToSymbol("bar").ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"hex list pattern": {
			input: `
			  a := [0xff, 0x26]
				switch a
				case \x[ff 26] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 36,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 27,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_16), 0, 255,
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 14,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 38,
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(72, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 50),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(255)).ToValue(), (value.SmallInt(38)).ToValue()}),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"bin list pattern": {
			input: `
			  a := [0b11, 0b10]
				switch a
				case \b[11 10] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 33,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 24,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_3),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 13,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(72, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 47),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(3)).ToValue(), (value.SmallInt(2)).ToValue()}),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"list pattern with rest elements": {
			input: `
			  a := [1, 5, [-2, 8, 3, 6]]
				switch a
				case [*b, [< 0, *c]] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 7,
					byte(bytecode.UNDEFINED),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.COPY),
					byte(bytecode.NEW_ARRAY_LIST8), 1,
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.UNDEFINED),
					byte(bytecode.UNDEFINED),
					byte(bytecode.NEW_ARRAY_LIST8), 0,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 136,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.INT_1),
					byte(bytecode.GREATER_EQUAL_I),
					byte(bytecode.JUMP_UNLESS_NP), 0, 125,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_3),
					byte(bytecode.INT_1),
					byte(bytecode.SUBTRACT_INT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.POP),
					byte(bytecode.INT_0),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_4),
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.GET_LOCAL_3),
					byte(bytecode.JUMP_UNLESS_ILT), 0, 15,
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.GET_LOCAL_2),
					byte(bytecode.SWAP),
					byte(bytecode.APPEND),
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.INCREMENT_INT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_4),
					byte(bytecode.POP),
					byte(bytecode.LOOP), 0, 20,
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.UNDEFINED),
					byte(bytecode.UNDEFINED),
					byte(bytecode.NEW_ARRAY_LIST8), 0,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 5,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 68,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 6,
					byte(bytecode.INT_1),
					byte(bytecode.GREATER_EQUAL_I),
					byte(bytecode.JUMP_UNLESS_NP), 0, 56,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 33,
					byte(bytecode.POP),
					byte(bytecode.INT_1),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 7,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL8), 7,
					byte(bytecode.GET_LOCAL8), 6,
					byte(bytecode.JUMP_UNLESS_ILT), 0, 19,
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL8), 7,
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.GET_LOCAL8), 5,
					byte(bytecode.SWAP),
					byte(bytecode.APPEND),
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL8), 7,
					byte(bytecode.INCREMENT_INT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 7,
					byte(bytecode.POP),
					byte(bytecode.LOOP), 0, 26,
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.INCREMENT_INT),
					byte(bytecode.SET_LOCAL_4),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(87, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 7),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 158),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(1)).ToValue(), (value.SmallInt(5)).ToValue()}),
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(-2)).ToValue(), (value.SmallInt(8)).ToValue(), (value.SmallInt(3)).ToValue(), (value.SmallInt(6)).ToValue()}),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"list pattern with unnamed rest elements": {
			input: `
			  a := [1, 5, [-2, 8, 3, 6]]
				switch a
				case [*, [< 0, *]] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 5,
					byte(bytecode.UNDEFINED),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.COPY),
					byte(bytecode.NEW_ARRAY_LIST8), 1,
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 77,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.INT_1),
					byte(bytecode.GREATER_EQUAL_I),
					byte(bytecode.JUMP_UNLESS_NP), 0, 66,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_2),
					byte(bytecode.INT_1),
					byte(bytecode.SUBTRACT_INT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL_3),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 41,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 4,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_4),
					byte(bytecode.INT_1),
					byte(bytecode.GREATER_EQUAL_I),
					byte(bytecode.JUMP_UNLESS_NP), 0, 30,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 7,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 5,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_3),
					byte(bytecode.INCREMENT_INT),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(85, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 7),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 92),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(1)).ToValue(), (value.SmallInt(5)).ToValue()}),
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(-2)).ToValue(), (value.SmallInt(8)).ToValue(), (value.SmallInt(3)).ToValue(), (value.SmallInt(6)).ToValue()}),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"tuple pattern": {
			input: `
			  a := %[1, 5, %[8, 3]]
				switch a
				case %[1, < 8, %[a, > 1 && < 5]] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 2,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 124,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_3),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 115,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 104,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 8,
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 80,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_2),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 64,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 55,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 44,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.GREATER),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 16,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(94, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 139),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayTupleOfValue{
						value.SmallInt(1).ToValue(),
						value.SmallInt(5).ToValue(),
						value.Ref(&value.ArrayTupleOfValue{
							value.SmallInt(8).ToValue(),
							value.SmallInt(3).ToValue(),
						}),
					}),
					value.Ref(value.TupleMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayTupleClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayTupleClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},

		"word tuple pattern": {
			input: `
			  a := %['foo', 'bar']
				switch a
				case %w[foo bar] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 34,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 25,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 14,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(77, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 49),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.NativeArrayTuple[value.String]{value.String("foo"), value.String("bar")}),
					value.Ref(value.TupleMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("foo")),
					value.Ref(value.String("bar")),
					value.Ref(value.String("a")),
				},
			),
		},
		"symbol tuple pattern": {
			input: `
			  a := %[:foo, :bar]
				switch a
				case %s[foo bar] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 34,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 25,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 14,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(75, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 49),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.NativeArrayTuple[value.Symbol]{value.ToSymbol("foo"), value.ToSymbol("bar")}),
					value.Ref(value.TupleMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.ToSymbol("foo").ToValue(),
					value.ToSymbol("bar").ToValue(),
					value.Ref(value.String("a")),
				},
			),
		},
		"hex tuple pattern": {
			input: `
			  a := %[0xff, 0x26]
				switch a
				case %x[ff 26] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 36,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 27,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_16), 0, 255,
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 14,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 38,
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(73, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 50),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayTupleOfValue{(value.SmallInt(255)).ToValue(), (value.SmallInt(38)).ToValue()}),
					value.Ref(value.TupleMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"bin tuple pattern": {
			input: `
			  a := %[0b11, 0b10]
				switch a
				case %b[11 10] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 1,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 33,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 24,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_3),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 13,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(73, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 47),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayTupleOfValue{(value.SmallInt(3)).ToValue(), (value.SmallInt(2)).ToValue()}),
					value.Ref(value.TupleMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"tuple pattern with rest elements": {
			input: `
			  a := %[1, 5, %[-2, 8, 3, 6]]
				switch a
				case %[*b, %[< 0, *c]] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 7,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.UNDEFINED),
					byte(bytecode.UNDEFINED),
					byte(bytecode.NEW_ARRAY_LIST8), 0,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 136,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.INT_1),
					byte(bytecode.GREATER_EQUAL_I),
					byte(bytecode.JUMP_UNLESS_NP), 0, 125,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_3),
					byte(bytecode.INT_1),
					byte(bytecode.SUBTRACT_INT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.POP),
					byte(bytecode.INT_0),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_4),
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.GET_LOCAL_3),
					byte(bytecode.JUMP_UNLESS_ILT), 0, 15,
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.GET_LOCAL_2),
					byte(bytecode.SWAP),
					byte(bytecode.APPEND),
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.INCREMENT_INT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_4),
					byte(bytecode.POP),
					byte(bytecode.LOOP), 0, 20,
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.UNDEFINED),
					byte(bytecode.UNDEFINED),
					byte(bytecode.NEW_ARRAY_LIST8), 0,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 5,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 68,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 6,
					byte(bytecode.INT_1),
					byte(bytecode.GREATER_EQUAL_I),
					byte(bytecode.JUMP_UNLESS_NP), 0, 56,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 33,
					byte(bytecode.POP),
					byte(bytecode.INT_1),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 7,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL8), 7,
					byte(bytecode.GET_LOCAL8), 6,
					byte(bytecode.JUMP_UNLESS_ILT), 0, 19,
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL8), 7,
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.GET_LOCAL8), 5,
					byte(bytecode.SWAP),
					byte(bytecode.APPEND),
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL8), 7,
					byte(bytecode.INCREMENT_INT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 7,
					byte(bytecode.POP),
					byte(bytecode.LOOP), 0, 26,
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.INCREMENT_INT),
					byte(bytecode.SET_LOCAL_4),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(91, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 158),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayTupleOfValue{
						value.SmallInt(1).ToValue(),
						value.SmallInt(5).ToValue(),
						value.Ref(&value.ArrayTupleOfValue{
							value.SmallInt(-2).ToValue(),
							value.SmallInt(8).ToValue(),
							value.SmallInt(3).ToValue(),
							value.SmallInt(6).ToValue(),
						}),
					}),
					value.Ref(value.TupleMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayTupleClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayTupleClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"tuple pattern with unnamed rest elements": {
			input: `
			  a := %[1, 5, %[-2, 8, 3, 6]]
				switch a
				case %[*, %[< 0, *]] then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 5,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 77,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.INT_1),
					byte(bytecode.GREATER_EQUAL_I),
					byte(bytecode.JUMP_UNLESS_NP), 0, 66,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_2),
					byte(bytecode.INT_1),
					byte(bytecode.SUBTRACT_INT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.GET_LOCAL_3),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 41,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_4),
					byte(bytecode.INT_1),
					byte(bytecode.GREATER_EQUAL_I),
					byte(bytecode.JUMP_UNLESS_NP), 0, 30,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 7,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_4),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL8), 5,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 5,
					byte(bytecode.POP),
					byte(bytecode.GET_LOCAL_3),
					byte(bytecode.INCREMENT_INT),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(89, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 2),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 92),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayTupleOfValue{
						value.SmallInt(1).ToValue(),
						value.SmallInt(5).ToValue(),
						value.Ref(&value.ArrayTupleOfValue{
							value.SmallInt(-2).ToValue(),
							value.SmallInt(8).ToValue(),
							value.SmallInt(3).ToValue(),
							value.SmallInt(6).ToValue(),
						}),
					}),
					value.Ref(value.TupleMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayTupleClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayTupleClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},

		"map pattern": {
			input: `
			  a := { 1 => 2, foo: :bar, "baz" => { dupa: [8, 3] } }
				switch a
				case { 1 => < 8, foo, "baz" => { dupa: [a, > 1 && < 5] } } then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 3,
					byte(bytecode.UNDEFINED),
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.UNDEFINED),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.COPY),
					byte(bytecode.NEW_HASH_MAP8), 1,
					byte(bytecode.NEW_HASH_MAP8), 1,
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 129,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 8,
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 105,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 6,
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 81,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 7,
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 64,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 8,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 55,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 44,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.GREATER),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 16,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 9,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(152, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 14),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 145),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(vm.MustNewHashMapOfValueWithCapacityAndElements(
						nil,
						3,
						value.MakePairOfValue(
							value.SmallInt(1).ToValue(),
							value.SmallInt(2).ToValue(),
						),
						value.MakePairOfValue(
							value.ToSymbol("foo").ToValue(),
							value.ToSymbol("bar").ToValue(),
						),
					)),
					value.Ref(value.String("baz")),
					vm.NewNativeKeyHashMap[value.Symbol](1).ToValue(),
					value.ToSymbol("dupa").ToValue(),
					value.Ref(&value.ArrayListOfValue{
						value.SmallInt(8).ToValue(),
						value.SmallInt(3).ToValue(),
					}),
					value.Ref(value.MapMixin),
					value.ToSymbol("foo").ToValue(),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"record pattern": {
			input: `
			  a := %{ 1 => 2, foo: :bar, "baz" => %{ dupa: [8, 3] } }
				switch a
				case %{ 1 => < 8, foo, "baz" => %{ dupa: [a, > 1 && < 5] } } then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 3,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.LOAD_VALUE_2),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.COPY),
					byte(bytecode.NEW_HASH_RECORD8), 1,
					byte(bytecode.NEW_HASH_RECORD8), 1,
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 133,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.CALL_METHOD8), 6,
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 8,
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 108,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 7,
					byte(bytecode.CALL_METHOD8), 8,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_1),
					byte(bytecode.CALL_METHOD8), 9,
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 82,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE_3),
					byte(bytecode.CALL_METHOD8), 10,
					byte(bytecode.DUP),
					byte(bytecode.LOAD_VALUE8), 11,
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 64,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 12,
					byte(bytecode.INT_2),
					byte(bytecode.EQUAL_INT),
					byte(bytecode.JUMP_UNLESS_NP), 0, 55,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_0),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 44,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.SUBSCRIPT),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.GREATER),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 16,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 13,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(156, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 12),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 149),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(vm.MustNewHashRecordOfValueWithElements(
						nil,
						value.MakePairOfValue(
							value.SmallInt(1).ToValue(),
							value.SmallInt(2).ToValue(),
						),
						value.MakePairOfValue(
							value.ToSymbol("foo").ToValue(),
							value.ToSymbol("bar").ToValue(),
						),
					)),
					value.Ref(value.String("baz")),
					vm.MakeNativeKeyHashRecord[value.Symbol](0).ToValue(),
					value.ToSymbol("dupa").ToValue(),
					value.Ref(&value.ArrayListOfValue{
						value.SmallInt(8).ToValue(),
						value.SmallInt(3).ToValue(),
					}),
					value.Ref(value.RecordMixin),
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("[]"), 1)),
					value.ToSymbol("foo").ToValue(),
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("[]"), 1)),
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("[]"), 1)),
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("[]"), 1)),
					value.Ref(value.ListMixin),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
		"object pattern": {
			input: `
			  a := [0b11, 0b10]
				switch a
				case ::Std::ArrayList(length: > 1 && < 5 as l, first) then "a"
				end
			`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 3,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_CONST8), 1,
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 50,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.GREATER),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 16,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.TRUE),
					byte(bytecode.JUMP_UNLESS), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.LOAD_VALUE8), 4,
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(111, 5, 8)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(4, 66),
					bytecode.NewLineInfo(3, 1),
					bytecode.NewLineInfo(5, 2),
				},
				[]value.Value{
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(3)).ToValue(), (value.SmallInt(2)).ToValue()}),
					value.ToSymbol("Std::ArrayList").ToValue(),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("first")).(*vm.NativeMethod), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
	}

	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			bytecodeCompilerTest(tc, t)
		})
	}
}

func TestBytecodeMatch(t *testing.T) {
	tests := bytecodeTestTable{
		"contains a pattern": {
			input: `
			a := [0b11, 0b10]
			if a match ::Std::ArrayList(length: > 1 && < 5 as l, first)
				puts "hooray"
			end
		`,
			want: vm.NewBytecodeFunctionNoParams(
				mainSymbol,
				[]byte{
					byte(bytecode.PREP_LOCALS8), 3,
					byte(bytecode.LOAD_VALUE_0),
					byte(bytecode.COPY),
					byte(bytecode.SET_LOCAL_1),
					byte(bytecode.GET_LOCAL_1),
					byte(bytecode.DUP),
					byte(bytecode.GET_CONST8), 1,
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS_NP), 0, 50,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 2,
					byte(bytecode.DUP),
					byte(bytecode.SET_LOCAL_2),
					byte(bytecode.DUP),
					byte(bytecode.INT_1),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.GREATER),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 16,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.INT_5),
					byte(bytecode.DUP_2),
					byte(bytecode.SWAP),
					byte(bytecode.GET_CLASS),
					byte(bytecode.IS_A),
					byte(bytecode.JUMP_UNLESS), 0, 4,
					byte(bytecode.LESS),
					byte(bytecode.JUMP), 0, 2,
					byte(bytecode.POP_2),
					byte(bytecode.FALSE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS_NP), 0, 6,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.CALL_METHOD_NT8), 3,
					byte(bytecode.SET_LOCAL_3),
					byte(bytecode.TRUE),
					byte(bytecode.POP_SKIP_ONE),
					byte(bytecode.JUMP_UNLESS), 0, 9,
					byte(bytecode.GET_CONST8), 4,
					byte(bytecode.LOAD_VALUE8), 5,
					byte(bytecode.CALL_METHOD_NT8), 6,
					byte(bytecode.JUMP), 0, 1,
					byte(bytecode.NIL),
					byte(bytecode.RETURN),
				},
				L(P(0, 1, 1), P(109, 5, 7)),
				bytecode.LineInfoList{
					bytecode.NewLineInfo(1, 2),
					bytecode.NewLineInfo(2, 3),
					bytecode.NewLineInfo(3, 62),
					bytecode.NewLineInfo(4, 6),
					bytecode.NewLineInfo(3, 4),
					bytecode.NewLineInfo(5, 1),
				},
				[]value.Value{
					value.Ref(&value.ArrayListOfValue{(value.SmallInt(3)).ToValue(), (value.SmallInt(2)).ToValue()}),
					value.ToSymbol("Std::ArrayList").ToValue(),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("length")).(*vm.NativeMethod), 0)),
					value.Ref(vm.NewNativeCallSiteInfo((value.ArrayListClass).LookupMethod(value.ToSymbol("first")).(*vm.NativeMethod), 0)),
					value.ToSymbol("Std::Kernel").ToValue(),
					value.Ref(value.String("hooray")),
					value.Ref(vm.NewNativeCallSiteInfo((value.KernelModule).SingletonClass().LookupMethod(value.ToSymbol("println@1")).(*vm.NativeMethod), 1)),
				},
			),
		},
	}

	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			bytecodeCompilerTest(tc, t)
		})
	}
}
