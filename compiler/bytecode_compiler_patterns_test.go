		...
		"hex set pattern": {
			...
			want: vm.NewBytecodeFunctionNoParams(
				...
				[]byte{
					...
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_16), 0x00, 0xFF,
					byte(bytecode.CALL_METHOD8), 3,
					byte(bytecode.JUMP_UNLESS_NP), 0, 11,
					byte(bytecode.POP),
					byte(bytecode.DUP),
					byte(bytecode.LOAD_INT_8), 0x26,
					byte(bytecode.CALL_METHOD8), 4,
					byte(bytecode.JUMP_UNLESS_NP), 0, 2,
					byte(bytecode.POP),
					byte(bytecode.TRUE),
		...
		"list pattern with rest elements": {
			...
			want: vm.NewBytecodeFunctionNoParams(
				...
				[]value.Value{
					...
					value.Ref(value.ListMixin),
					value.Ref(&vm.CallSiteInfo{Name: value.ToSymbol("length")}),
					value.Ref(&vm.CallSiteInfo{Name: value.ToSymbol("length")}),
					value.Ref(value.String("a")),
				},
			),
		...
		"object pattern": {
			...
			want: vm.NewBytecodeFunctionNoParams(
				...
				[]value.Value{
					...
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("length"), 0)),
					value.Ref(vm.NewCallSiteInfo(value.ToSymbol("first"), 0)),
					value.Ref(value.String("a")),
				},
			),
		},
