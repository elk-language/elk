...
func TestUnicodeCharClass(t *testing.T) {
	tests := testTable{
		...
		"invalid multi-letter": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(8, 1, 9), P(8, 1, 9)), "unexpected 9, expected an alphabetic character"),
			},
		},
		"missing end brace": {
			input: `\p{Latin`,
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(8, 1, 9), P(7, 1, 8)), "unexpected END_OF_FILE, expected an alphabetic character"),
			},
		},
		"invalid single char": {
			input: `\p'`,
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(2, 1, 3), P(2, 1, 3)), "unexpected ', expected an alphabetic character"),
				diagnostic.NewFailure(L("regex", P(2, 1, 3), P(2, 1, 3)), "unexpected ', expected an alphabetic character"),
			},
		},
		"missing single char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(2, 1, 3), P(1, 1, 2)), "unexpected END_OF_FILE, expected an alphabetic character"),
			},
		},
	}
...
func TestQuantifier(t *testing.T) {
	tests := testTable{
		...
		"N quantifier invalid chars": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(3, 1, 4), P(3, 1, 4)), "unexpected f, expected a decimal digit"),
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(4, 1, 5)), "unexpected +, expected a decimal digit"),
			},
		},
		"N quantifier missing right brace": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(3, 1, 4), P(2, 1, 3)), "unexpected END_OF_FILE, expected }"),
			},
		},
		"N quantifier missing digit": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(2, 1, 3), P(2, 1, 3)), "expected decimal digits"),
			},
		},
		"N quantifier missing digit and right brace": {
			...
			err: diagnostic.DiagnosticList{
				...
				diagnostic.NewFailure(L("regex", P(1, 1, 2), P(1, 1, 2)), "expected decimal digits"),
			},
		},
		...
		"NM quantifier N only missing right brace": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(3, 1, 4)), "unexpected END_OF_FILE, expected }"),
			},
		},
		...
		"NM quantifier missing right brace": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(8, 1, 9), P(7, 1, 8)), "unexpected END_OF_FILE, expected }"),
			},
		},
		"NM quantifier invalid chars": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(2, 1, 3), P(2, 1, 3)), "unexpected a, expected a decimal digit"),
				diagnostic.NewFailure(L("regex", P(6, 1, 7), P(6, 1, 7)), "unexpected f, expected a decimal digit"),
			},
		},
	}
...
func TestCaretEscape(t *testing.T) {
	tests := testTable{
		...
		"invalid char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(2, 1, 3), P(3, 1, 3)), "unexpected ę, expected an ASCII letter"),
			},
		},
	}
...
func TestUnicodeEscape(t *testing.T) {
	tests := testTable{
		...
		"four digit with invalid char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(5, 1, 6), P(5, 1, 6)), "unexpected l, expected a hex digit"),
			},
		},
		"four digit with invalid meta char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(5, 1, 6), P(5, 1, 6)), "unexpected {, expected a hex digit"),
			},
		},
		"missing digit": {
			input: `\ufff`,
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(5, 1, 6), P(4, 1, 5)), "unexpected END_OF_FILE, expected a hex digit"),
			},
		},
		"with braces": {
			input: `\u{6f}`,
		...
		"with braces and invalid chars": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(4, 1, 5)), "unexpected ., expected a hex digit"),
				diagnostic.NewFailure(L("regex", P(6, 1, 7), P(6, 1, 7)), "unexpected {, expected a hex digit"),
			},
		},
	}
...
func TestLongUnicodeEscape(t *testing.T) {
	tests := testTable{
		...
		"eight digit with invalid char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(9, 1, 10), P(9, 1, 10)), "unexpected l, expected a hex digit"),
			},
		},
		"eight digit with invalid meta char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(9, 1, 10), P(9, 1, 10)), "unexpected {, expected a hex digit"),
			},
		},
		"missing digit": {
			input: `\U0000fff`,
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(9, 1, 10), P(8, 1, 9)), "unexpected END_OF_FILE, expected a hex digit"),
			},
		},
		"with braces": {
			input: `\U{6f}`,
...
func TestHexEscape(t *testing.T) {
	tests := testTable{
		...
		"two digit with invalid char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(3, 1, 4), P(3, 1, 4)), "unexpected l, expected a hex digit"),
			},
		},
		"two digit with invalid meta char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(3, 1, 4), P(3, 1, 4)), "unexpected {, expected a hex digit"),
			},
		},
		"missing digit": {
			input: `\xf`,
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(3, 1, 4), P(2, 1, 3)), "unexpected END_OF_FILE, expected a hex digit"),
			},
		},
		"with braces": {
			input: `\x{6f}`,
...
func TestOctalEscape(t *testing.T) {
	tests := testTable{
		...
		"two digit with invalid char": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(3, 1, 4), P(3, 1, 4)), "unexpected 9, expected an octal digit"),
			},
		},
		"two digit with invalid meta char": {
			input: `\o6{`,
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(3, 1, 4), P(3, 1, 4)), "unexpected {, expected an octal digit"),
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(3, 1, 4)), "unexpected END_OF_FILE, expected an octal digit"),
			},
		},
		"missing digit": {
			input: `\o72`,
		...
		"missing end brace": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(5, 1, 6), P(4, 1, 5)), "unexpected END_OF_FILE, expected an octal digit"),
			},
		},
		"long with braces": {
			input: `\o{612}`,
		...
		"with braces and too long": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(0, 1, 1), P(6, 1, 7)), "too many octal digits"),
			},
		},
	}
...
func TestSimpleCharClass(t *testing.T) {
	tests := testTable{
		...
		"not word in negated char class": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(5, 1, 6)), `double negation of unicode-aware \W is not supported`),
			},
		},
		"not word ascii": {
			input: `\W`,
		...
		"not whitespace in negated char class": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(5, 1, 6)), `double negation of unicode-aware \S is not supported`),
			},
		},
		...
		"not horizontal whitespace in negated char class": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(5, 1, 6), P(6, 1, 7)), `double negation of unicode-aware \H is not supported`),
			},
		},
		...
		"not vertical whitespace in negated char class": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(5, 1, 6), P(6, 1, 7)), `double negation of unicode-aware \V is not supported`),
			},
		},
...
func TestQuotedText(t *testing.T) {
	tests := testTable{
		...
		"invalid escapes": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(0, 1, 1), P(2, 1, 3)), "expected end of quoted text"),
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(5, 1, 6)), "invalid escape sequence: \\E"),
			},
		},
		"in char class": {
			input: `[\Q\e\E]`,
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(1, 1, 2), P(3, 1, 4)), "expected end of quoted text"),
				diagnostic.NewFailure(L("regex", P(5, 1, 6), P(6, 1, 7)), "invalid escape sequence: \\E"),
			},
		},
	}
...
func TestCharClass(t *testing.T) {
	tests := testTable{
		...
		"unterminated": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(3, 1, 4)), "unterminated character class, missing ]"),
			},
		},
		"invalid chars": {
			input: "[-]",
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(1, 1, 2), P(1, 1, 2)), "unexpected -, expected a char class element"),
			},
		},
		"char ranges": {
			input: `[a-z\n-\r\x22-\x7f56]`,
		...
		"named char class with invalid chars": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(7, 1, 8), P(8, 1, 8)), "unexpected ę, expected an ASCII letter"),
			},
		},
...
func TestGroup(t *testing.T) {
	tests := testTable{
		...
		"invalid flags": {
			...
			want:  ``,
			err: diagnostic.DiagnosticList{
				diagnostic.NewFailure(L("regex", P(4, 1, 5), P(4, 1, 5)), "unexpected h, expected a regex flag"),
			},
		},
		"char in group": {
			input: "(f)",
