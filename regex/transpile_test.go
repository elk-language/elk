...
func TestSimpleCharClass(t *testing.T) {
	tests := testTable{
		...
		"not word in negated char class": {
			...
			want:  ``,
			err: errors.ErrorList{
				errors.NewError(L("regex", P(4, 1, 5), P(5, 1, 6)), `double negation of unicode-aware \W is not supported`),
			},
		},
		"not word ascii": {
			input:     `\W`,
		...
		"not whitespace in negated char class": {
			...
			want:  ``,
			err: errors.ErrorList{
				errors.NewError(L("regex", P(4, 1, 5), P(5, 1, 6)), `double negation of unicode-aware \S is not supported`),
			},
		},
...
func TestQuotedText(t *testing.T) {
	tests := testTable{
		...
		"in char class": {
			...
			want:  ``,
			err: errors.ErrorList{
				errors.NewError(L("regex", P(1, 1, 2), P(3, 1, 4)), "expected end of quoted text"),
				errors.NewError(L("regex", P(5, 1, 6), P(6, 1, 7)), "invalid escape sequence: \\E"),
			},
		},
	}
